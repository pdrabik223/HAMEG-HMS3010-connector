...
class Device:
    ...
    @staticmethod
    def connect_via_vid_pid(idVendor, idProduct):
        logging.debug(f"connecting do device with idProduct: {idProduct}, idVendor: {idVendor}")
        ...
        logging.debug(f"connected do device with idProduct: {idProduct}, idVendor: {idVendor}")
        return dev
    ...
    def _send_str(self, command: str):
        ...
        try:
            ...
            raise Exception(f"error occurred while writing to device: {str(ex)}")
